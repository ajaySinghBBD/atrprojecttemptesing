...
//        verify(donationPostRepository.delete(donationPost), times(1));
    ...
    @Test
    void getDonationPostDetails() {


        DonationPost donationPost = new DonationPost();
        donationPost.setPostId(2L);

        DonationPostDetailsDTO  donationPostDetailsDTO = new DonationPostDetailsDTO();
        //mock
        when(donationPostRepository.findById(2L)).thenReturn(Optional.of(donationPost));
        //call
        createDonationService.getDonationPostDetails(2l);

//        DonationPost donationPost1 =
//        donationPost1.setPostId(3L);

        List<DonationPost> donationPosts = donationPostRepository.findAll();
        //assert
        Assertions.assertThat(donationPosts.size()).isEqualTo(2);
        Assertions.assertThat(donationPosts.size()).isGreaterThan(0);
